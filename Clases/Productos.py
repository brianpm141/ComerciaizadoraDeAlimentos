        ...
        with open(ruta_archivo, mode='r') as file:
            ...
            for row in reader:
                if row:
                    ...
                    producto = Producto(
                        id, nombre, tipo, peso, precio, cantidad)
...
def guardar_en_csv(lista_productos):
    ruta_archivo = Path(__file__).parent.parent / 'Archivos' / 'productos.csv'
    with open(ruta_archivo, mode='w', newline='') as file:
        writer = csv.writer(file)
        # Escribir la cabecera del CSV
        writer.writerow(["ID", "Nombre", "Tipo", "Peso",
                        "Precio", "Cantidad", "enpedido", "Status"])
        # Escribir los datos de cada producto
        for producto in lista_productos:
            writer.writerow([producto.getid(), producto.getnombre(), producto.gettipo(), producto.getpeso(
            ), producto.getprecio(), producto.getcantidad(), producto.getenpedido(), producto.getstatus()])
...
def generarcsv(ruta):
    ...
    with open(ruta_archivo, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["ID", "Nombre", "Tipo", "Peso",
                        "Precio", "Cantidad", "En pedido"])
        for producto in lista_produtos:
            writer.writerow([producto.getid(), producto.getnombre(), producto.gettipo(
            ), producto.getpeso(), producto.getprecio(), producto.getcantidad(), producto.getenpedido()])


def generarexel(ruta):
    columnas = ["ID", "Nombre", "Tipo", "Peso",
                "Precio", "Cantidad", "En pedido"]
    datos = [
        [producto.getid(), producto.getnombre(), producto.gettipo(), producto.getpeso(
        ), producto.getprecio(), producto.getcantidad(), producto.getenpedido()]
        for producto in lista_produtos
    ]
