...
class Pedido(Venta):
    def __init__(self, id, fecha, hora, id_usuario, id_producto, cantidad,  subtotal, metodo_pago, estado, id_cliente, fecha_entrega):
        ...
        super().__init__(id, fecha, hora, id_usuario,
        ...
        with open(ruta_archivo, mode='r') as file:
            ...
            for row in reader:
                if row:
                    ...
                    pedtaux = Pedido(id, fecha, hora, id_usuario, id_producto, cantidad,
                                     subtotal, metodo_pago, estado, id_cliente, fecha_entrega)
...
def guardar_en_csv(lista_pedidos):
    ruta_archivo = Path(__file__).parent.parent / 'Archivos' / 'pedidos.csv'

    with open(ruta_archivo, mode='w', newline='') as file:
        writer = csv.writer(file)
        # Escribir la cabecera del CSV
        writer.writerow(["id", "fecha", "hora", "id_usuario", "id_producto", "cantidad",
                        "subtotal", "metodo_pago", "estado", "id_cliente", "fecha_entrega"])
        for pedido in lista_pedidos:
            writer.writerow([pedido.getid(), pedido.getfecha(), pedido.gethora(), pedido.getid_usuario(), pedido.getid_producto(),
                             pedido.getcantidad(), pedido.getsubtotal(
            ), pedido.getmetodo_pago(), pedido.getestado(),
...
def generarcsv(ruta):
    with open(ruta, mode='w', newline='') as file:
        ...
        for venta in listaPedidos:
            writer.writerow([venta.getid(), venta.getfecha(), venta.gethora(), venta.getid_usuario(), venta.getid_producto(), venta.getcantidad(), venta.getsubtotal(), venta.getmetodo_pago(), venta.getestado(),
                             venta.getid_cliente(), venta.getfecha_entrega()])


def gengerarexel(ruta):
    # Definir las columnas del DataFrame
    columnas = ["id", "fecha", "hora", "id_usuario", "id_producto", "cantidad",
                "subtotal", "metodo_pago", "estado", "id_cliente", "fecha_entrega"]
